...
public class RetrieveScoreController {
    ...
    public GeneralContentResult<SimplePersonalScoreDetail> getPersoanlAnswerScoreList(@ApiParam(value = "试卷的ID") @PathVariable(value = "_assessPaperId", required = true) String _assessPaperId,
                                                                                      @ApiParam(value = "做题人的ID, 为空则表示当前用户") @RequestParam(value = "_userId", required = false) String _userId){
        GeneralContentResult<SimplePersonalScoreDetail> result  = new GeneralContentResult<>();
