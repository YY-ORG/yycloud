...
//        List<IPerAssespaperPeriod> tempPeriodList = this.perAssessPaperRepository.getAssessPaperPeriodByPage();
        ...
        tempResult.setResultCode(ResultCode.OPERATION_SUCCESS);
        tempResult.setResultContent(tempPeriodItemList);

        PageInfo tempPage = new PageInfo();
        tempPage.setCurrentPage(_page.getPageNumber());
        tempPage.setPageSize(_page.getPageSize());
        tempPage.setTotalPage(tempAssessPeriodPage.getTotalPages());
        tempPage.setTotalRecords(tempAssessPeriodPage.getTotalElements());
    ...
    public GeneralResult updateAssessPeriodList(List<AssessPeriodReq> _reqList, String _userId) {
        if (_reqList != null && _reqList.size() > 0) {
            _reqList.stream().forEach(item -> this.updateAssessPeriod(item, _userId));
        }
        GeneralResult tempResult = new GeneralResult();
        tempResult.setResultCode(ResultCode.OPERATION_SUCCESS);
        return tempResult;
    ...
    public GeneralResult updateAssessPeriod(AssessPeriodReq _req, String _userId) {
        PerAssessPeriod tempAssessPeriod = null;
        if (StringUtils.isNotBlank(_req.getPeriodId())) {
            tempAssessPeriod = this.perAssessPeriodRepository.findOne(_req.getPeriodId());
        }
        //one assess paper has only one period.
        if (tempAssessPeriod == null) {
            tempAssessPeriod = this.perAssessPeriodRepository.getPerAssessPeriodByAssessPaperId(_req.getAssessPaperId());
        }
        if (tempAssessPeriod == null) {
            tempAssessPeriod = new PerAssessPeriod();
        }
        tempAssessPeriod.setAssessPaperId(_req.getAssessPaperId());
        tempAssessPeriod.setDoingStart(_req.getExStartTime());
        tempAssessPeriod.setDoingEnd(_req.getExEndTime());
        tempAssessPeriod.setScStart(_req.getScStartTime());
        tempAssessPeriod.setScEnd(_req.getScEndTime());
        tempAssessPeriod.setAuStart(_req.getAuStartTime());
        tempAssessPeriod.setAuEnd(_req.getAuEndTime());
        tempAssessPeriod.setCreatorId(_userId);
        this.perAssessPeriodRepository.save(tempAssessPeriod);
        GeneralResult tempResult = new GeneralResult();
        tempResult.setResultCode(ResultCode.OPERATION_SUCCESS);
        return tempResult;
    }
    ...
    private AssessPeriodItem convertToPeriodItem(IPerAssespaperPeriod _org) {
        ...
        tempItem.setScStartTime(_org.getScStartTime());
        tempItem.setScEndTime(_org.getScEndTime());
        tempItem.setAuStartTime(_org.getAuStartTime());
        tempItem.setAuEndTime(_org.getAuEndTime());
        return tempItem;
    }
}
